...
import { MapPin, LayoutGrid, Calendar, ArrowRight } from "lucide-react";
           ...
           {projects.map((project) => (
             <Link key={project.id} href={`/projects/${project.id}`} className="retail-card group flex flex-col hover:shadow-xl transition-shadow duration-300">
                <div className="aspect-[16/10] bg-ag-bg-alt relative overflow-hidden flex items-center justify-center">
                   <div className="w-full h-full bg-ag-border/20 flex items-center justify-center text-ag-text-muted/10 font-heading font-black text-6xl italic italic animate-pulse">
                      {project.surface.charAt(0)}
                   </div>
