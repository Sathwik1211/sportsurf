        ...
        <div className="max-w-3xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="space-y-2 text-center md:text-left">
            ...
            <h2 className="font-heading font-bold text-white text-2xl md:text-3xl">
              {settings?.ctaTitle || "Ready to Build?"}
            </h2>
            <p className="font-body text-white/60 text-sm max-w-md">
              {settings?.ctaSubtitle || "Contact us to get a complete guide & quote."}
            </p>
          </div>

          <div className="flex flex-col gap-3 shrink-0">
            <Link href={settings?.ctaLink || "/contact"} className="btn btn-gold px-10 py-3 text-[11px] rounded-none">
              {settings?.ctaButton || "REQUEST FREE ESTIMATE"}
            </Link>
